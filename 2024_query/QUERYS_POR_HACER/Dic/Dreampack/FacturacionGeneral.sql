...
SELECT 
    ...
    CASE 
        WHEN T0."NumAtCard" LIKE '04-001%' THEN ((CASE WHEN T1."NoInvtryMv" = 'N' THEN T1."Quantity" ELSE 0 END) * -1) 
        ELSE T1."Quantity" 
    END AS "CANTIDAD",
    ...
    T1."InvQty" AS "Cant Conv", (T1."InvQty" * T2."U_SYP_PESOBRUTO") AS "KG",
    ((T1."InvQty" * T2."U_SYP_PESOBRUTO") / 1000) AS "Ton", 
    (((T1."InvQty" * T2."U_SYP_PESOBRUTO") / 1000) * T1."PriceBefDi") AS "D/T", 
    T4."Name" AS "SG3", 
    T5."Name" AS "SG4"
FROM OINV T0  
...
LEFT JOIN "SBO_FIGURETTI_PRO"."@SYP_SUBGRUPO3" T4 ON T2."U_SYP_SUBGRUPO3" = T4."Code"
LEFT JOIN "SBO_FIGURETTI_PRO"."@SYP_SUBGRUPO4" T5 ON T2."U_SYP_SUBGRUPO4" = T5."Code"
WHERE T0."CANCELED" = 'N' AND  T0."DocDate" BETWEEN [%0] AND [%1]
UNION ALL
SELECT T0."DocEntry", T0."DocNum", T0."U_LAB_ORDCOM", T0."DocDate", T0."CardName", T0."NumAtCard", T1."ItemCode", T1."Dscription", T1."AcctCode"  AS "Cuenta Contable", 
CASE WHEN T0."NumAtCard" LIKE '04-001%' THEN ((CASE WHEN T1."NoInvtryMv" = 'N' THEN T1."Quantity" ELSE 0 END) * -1) ELSE T1."Quantity" END AS "CANTIDAD", T1."PriceBefDi" As "Precio Unitario", T1."StockPrice", T1."UomCode"  AS "UoM", 
CASE WHEN T0."NumAtCard" LIKE '04-001%' THEN (T1."TotalSumSy" * -1) ELSE T1."TotalSumSy" END AS "TOTAL",  T1."TaxCode" AS "IVA", T1."GTotalSC" AS "Total Bruto", T2."SlpName" AS "Vendedor",
T3."U_SYP_PESOBRUTO" AS "PESO BRUTO", T1."NoInvtryMv" AS "No Mueve Inventario", T1."InvQty" AS "Cant Conv", (T1."InvQty" * T3."U_SYP_PESOBRUTO") AS "KG",
((T1."InvQty" * T3."U_SYP_PESOBRUTO") / 1000) AS "Ton", (((T1."InvQty" * T3."U_SYP_PESOBRUTO") / 1000) * T1."PriceBefDi") AS "D/T", T4."Name" AS "SG3", T5."Name" AS "SG4"
FROM ORIN T0
INNER JOIN RIN1 T1 ON T0."DocEntry" = T1."DocEntry"
INNER JOIN OSLP T2 ON T0."SlpCode" = T2."SlpCode"
LEFT JOIN OITM T3 ON T1."ItemCode" = T3."ItemCode"
LEFT JOIN "SBO_FIGURETTI_PRO"."@SYP_SUBGRUPO3" T4 ON T3."U_SYP_SUBGRUPO3" = T4."Code"
LEFT JOIN "SBO_FIGURETTI_PRO"."@SYP_SUBGRUPO4" T5 ON T3."U_SYP_SUB"



/* Factuacion General 2 */
...
SELECT 
    ...
    (T1."Quantity" * T1."StockPrice") AS "Total Costo", 
    T1."TaxCode"  AS "IVA", 
    T1."GTotalSC" AS "Total Bruto", 
    T3."SlpName" AS "Vendedor",
    T2."U_SYP_PESOBRUTO" AS "PESO BRUTO", 
...
SELECT 
    ...
    CASE 
        ...
        ELSE T1."TotalSumSy" 
    END AS "Total Ingreso", 
    (T1."Quantity" * T1."StockPrice") * -1 AS "Total Costo", 
    T1."TaxCode" AS "IVA", 
...
/* Factuacion General 2  se va a modificar el D/T 
-- "Total Ingreso" divido para el resultado de (Cantidad Venc * peso bruto) / 1000 
Falta validarlo 24-12-2024
*/
...
SELECT 
    ...
    (((T1."InvQty" * COALESCE(A1."U_SYP_UPPL", 1)) * (T2."U_SYP_PESOBRUTO" / COALESCE(A1."U_SYP_UPPL", 1))) / 1000) AS "Ton", 
    CASE
        WHEN ( (T1."InvQty" * COALESCE(A1."U_SYP_UPPL", 1)) * (T2."U_SYP_PESOBRUTO") / 1000 ) <> 0 THEN
            CASE WHEN T0."NumAtCard" LIKE '04-001%' THEN (T1."TotalSumSy" * -1)
                 ELSE T1."TotalSumSy"
            END / ((T1."InvQty" * COALESCE(A1."U_SYP_UPPL", 1)) * (T2."U_SYP_PESOBRUTO") / 1000)
        ELSE 0
    END AS "D/T",
    --((((T1."InvQty" * COALESCE(A1."U_SYP_UPPL", 1)) * (T2."U_SYP_PESOBRUTO" / COALESCE(A1."U_SYP_UPPL", 1))) / 1000) * T1."PriceBefDi") AS "D/T", 
    T4."Name" AS "SG1", 
...
SELECT 
    ...
    -- Cálculo D/T
    CASE
        WHEN (CASE WHEN T1."NoInvtryMv" = 'N' THEN (T1."InvQty" * COALESCE(A1."U_SYP_UPPL", 1) * -1) ELSE 0 END) * (T3."U_SYP_PESOBRUTO") / 1000 <> 0 THEN
            CASE WHEN T0."NumAtCard" LIKE '04-001%' THEN (T1."TotalSumSy" * -1)
                 ELSE T1."TotalSumSy"
            END / ((CASE WHEN T1."NoInvtryMv" = 'N' THEN (T1."InvQty" * COALESCE(A1."U_SYP_UPPL", 1) * -1) ELSE 0 END) * (T3."U_SYP_PESOBRUTO") / 1000)
        ELSE 0
    END AS "D/T",
