...
IF :object_type = '22' AND (:transaction_type = 'A' OR :transaction_type = 'U') THEN 
    ...
    INTO 
        ...
        ADDRESS_VALID, STREET_VALID, COLONIA_VALID, CITY_VALID, ZIP_CODE_VALID, COUNTRY_VALID,
        PAYMENT_CONDITION_VALID, LIC_TRAD_NUM_VALID, ID_FISCAL_VALID, REGIMEN_FISCAL_V4_VALID,
        PAYMENT_METHOD_CHECK_VALID, ANEXOS_VALID , SAL_UNIT_MSR_VALID, INVNTRY_UOM_VALID
    FROM OCRD T0
    LEFT JOIN CRD1 T1 ON T0."CardCode" = T1."CardCode"
    LEFT JOIN OCRY T2 ON T1."Country" = T2."Code"
    LEFT JOIN OPYM T3 ON T0."PymCode" = T3."PayMethCod"
...
/*  solo cuando es cmercio exterior debe ir id fiscal*/

 -- Validación del ID fiscal (VatIdUnCmp)
        MAX(CASE 
            WHEN T0."QryGroup1" = 'Y' AND (T0."VatIdUnCmp" IS NULL OR T0."VatIdUnCmp" = '') THEN 1 
            ELSE 0 
...
IF :object_type = '22' AND (:transaction_type = 'A' OR :transaction_type = 'U') THEN 
    ...
    -- Validaciones previas (socio de negocio)
    -- ... [Tu código existente para validar el socio de negocio] ...
    ...
    BEGIN
        ...
        AND (
            ...
            (A2."SellItem" = 'Y' AND (A2."InvntryUom" IS NULL OR A2."InvntryUom" = '' OR A2."InvntryUom" NOT IN ('XUN', 'KGM', 'PLIEGO', 'XBX', 'H87', 'ACT')))
        );

        IF INVALID_ARTICLE_COUNT > 0 THEN
            error := 8;  
            error_message := N'Existen artículos inválidos en la orden de compra. Verifique las unidades de medida.';
            RETURN;
        END IF;
...
IF :object_type = '22' AND (:transaction_type = 'A' OR :transaction_type = 'U') THEN 
    ...
    SELECT
	    ...
	    -- Validación de artículos
	    COUNT(CASE 
	        WHEN A2."SellItem" = 'Y' AND (A2."SalUnitMsr" IS NULL OR A2."SalUnitMsr" = '' OR A2."SalUnitMsr" NOT IN ('XUN', 'KGM', 'PLIEGO', 'XBX', 'H87', 'ACT')) THEN 1
	        WHEN A2."SellItem" = 'Y' AND (A2."InvntryUom" IS NULL OR A2."InvntryUom" = '' OR A2."InvntryUom" NOT IN ('XUN', 'KGM', 'PLIEGO', 'XBX', 'H87', 'ACT')) THEN 2
	        ELSE NULL
	    END) AS InvalidArticleCount

    INTO 
        BP_EXISTS, CARD_NAME_VALID, EMAIL_VALID, PHONE_VALID, MAIN_USAGE, 
        ADDRESS_VALID, STREET_VALID, COLONIA_VALID, CITY_VALID, ZIP_CODE_VALID, COUNTRY_VALID,
        PAYMENT_CONDITION_VALID, LIC_TRAD_NUM_VALID, ID_FISCAL_VALID, REGIMEN_FISCAL_V4_VALID,
        PAYMENT_METHOD_CHECK_VALID, INVALID_ARTICLE_COUNT
    ...
    LEFT JOIN OPYM T3 ON T0."PymCode" = T3."PayMethCod"
	LEFT JOIN CRD2 T5 ON T0."CardCode" = T5."CardCode"
	LEFT JOIN POR1 A1 ON A0.DocEntry = A1.DocEntry
	LEFT JOIN OITM A2 ON A1.ItemCode = A2.ItemCode
    WHERE A0.DocEntry IN (:list_of_cols_val_tab_del);

	-- Asignar un error general si alguna validación falla en el socio de negocio o en los artículos
    IF BP_EXISTS > 0 OR CARD_NAME_VALID > 0 
    	OR EMAIL_VALID > 0 OR PHONE_VALID > 0 
    	OR MAIN_USAGE > 0 OR ADDRESS_VALID > 0 
    	OR STREET_VALID > 0 OR COLONIA_VALID > 0 
    	...
    	OR PAYMENT_CONDITION_VALID > 0 OR LIC_TRAD_NUM_VALID > 0 
    	OR ID_FISCAL_VALID > 0 OR REGIMEN_FISCAL_V4_VALID > 0 
    	OR PAYMENT_METHOD_CHECK_VALID > 0 
    	OR INVALID_ARTICLE_COUNT > 0 THEN
        error := (CASE 
                     WHEN INVALID_ARTICLE_COUNT > 0 THEN
                         -- Error específico para artículos inválidos.
                         N'Existen artículos inválidos en la orden de compra. Verifique las unidades de medida.'
                     ELSE
                         -- Error general para otros campos.
                         N'Faltan campos por validar en el maestro socio negocio antes de crear la orden de compra.'
                   END);
...
-- Validación de artículos en POR1
DECLARE
    INVALID_ARTICLE_COUNT INT;
BEGIN
    SELECT COUNT(*)
...
/* ASI QUEDO EN PRUEBA 22-10-2024 */
DROP PROCEDURE SBO_SP_TransactionNotification_CLIENT;
...
IF :object_type = '2' AND (:transaction_type = 'A' OR :transaction_type = 'U') THEN
	...
	SELECT
        ...
        MAX(CASE 
		    ...
		    --WHEN LENGTH(T0."Phone1") < 10 THEN 2  -- Teléfono con menos de 10 caracteres
		    --WHEN LENGTH(T0."Phone1") > 15 THEN 3  -- Teléfono con más de 15 caracteres
		    WHEN T0."Phone1" NOT LIKE_REGEXPR '^\+?[0-9]+$' /*'^[0-9]+$'*/ THEN 2  -- Teléfono contiene caracteres no numéricos
		    ELSE 0 
		END) AS PhoneCheck,
        MAX(CASE WHEN T0."U_B1SYS_MainUsage" IS NULL OR T0."U_B1SYS_MainUsage" = '' THEN 1 ELSE 0 END) AS MainUsageCheck,
        MAX(CASE WHEN T1."Address" IS NULL OR T1."Address" = '' THEN 1 ELSE 0 END) AS AddressCheck,
...
IF :object_type = '22' AND (:transaction_type = 'A' OR :transaction_type = 'U') THEN 
    ...
    INTO 
        ...
        ADDRESS_VALID, STREET_VALID, COLONIA_VALID, CITY_VALID, ZIP_CODE_VALID, COUNTRY_VALID,
        PAYMENT_CONDITION_VALID, LIC_TRAD_NUM_VALID, ID_FISCAL_VALID, REGIMEN_FISCAL_V4_VALID,
        PAYMENT_METHOD_CHECK_VALID, ANEXOS_VALID --, INVALID_ARTICLE_COUNT --, SAL_UNIT_MSR_VALID, INVNTRY_UOM_VALID
...
/* ASI QUEDO EN PRODUCCION 22-10-2024 */
